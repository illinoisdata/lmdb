...
int main(int argc, char* argv[]) {
    ...
    for (size_t t_idx = 0; t_idx < num_samples; t_idx++) {
        ...
        if (type == 'r') {  // READ
            ...
            // check correctness
            if (rcv_key != key && rcv_rank > 0) {
                printf("ERROR: incorrect rcv_rank= %lu with rcv_key= %lu, expected: key= %lu\n", rcv_rank, rcv_key, key);
            }
        } else if (type == 'w') {  // WRITE
            encode64(0, dval); lmdb_data.mv_size = sizeof(uint64_t); lmdb_data.mv_data = dval;
            int result = mdb_put(txn, dbi, &lmdb_key, &lmdb_data, MDB_NOOVERWRITE);
            if (RES(MDB_KEYEXIST, result)) {
                printf("ERROR: duplicated key= %lu not inserted\n", key);
            }
        } else {
